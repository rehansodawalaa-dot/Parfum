...
import { User, ShoppingBag, Copy, LogOut, Package, Star, CreditCard, ChevronRight } from 'lucide-react';
...
function OrdersTab({ userId }) {
  ...
  // Demo fallback
  const DEMO_ORDERS = [
    { _id: '1', orderNumber: 'JRS-20250502-DEMO1', createdAt: new Date('2025-05-02'), items: [{ name: 'Noir Absolu', quantity: 1 }, { name: 'Rose Éternelle', quantity: 1 }], total: 15700, status: 'delivered' },
    { _id: '2', orderNumber: 'JRS-20250418-DEMO2', createdAt: new Date('2025-04-18'), items: [{ name: 'Velvet Oud', quantity: 1 }], total: 18500, status: 'delivered' },
    { _id: '3', orderNumber: 'JRS-20250303-DEMO3', createdAt: new Date('2025-03-03'), items: [{ name: 'Bois Sacré', quantity: 2 }, { name: 'Soleil Doré', quantity: 1 }], total: 26000, status: 'delivered' },
  ];
  ...
  if (orders.length === 0) {
    return (
      <div className="text-center py-20">
        ...
        <p className="text-stone-400 text-sm font-sans mb-6">Your order history will appear here.</p>
        <Link to="/shop" className="btn-dark inline-flex">Start Shopping</Link>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-3">
      {orders.map((order) => (
        <div key={order._id} className="bg-white border border-stone-100 p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:border-stone-200 transition-colors">
...
export default function Account() {
  ...
  return (
    <>
      <SEO title="My Account" description="Manage your J Raph Streach orders, profile, and referrals." />
              ...
              <div className="space-y-5">
                {[
                  ...
                  <div key={f.label}>
                    ...
                    <p className="font-sans text-sm text-obsidian bg-stone-50 border border-stone-100 px-4 py-3 capitalize">{f.value}</p>
                  </div>
                ))}
              </div>
            ...
            <div className="max-w-lg">
              <div className="bg-obsidian p-8 mb-6">
                ...
                <div className="flex items-center gap-3 bg-white/5 border border-cream/20 px-5 py-4 mb-4">
                  ...
                  <button onClick={copyReferral} className="text-cream/50 hover:text-gold-400 transition-colors" aria-label="Copy">
                    <Copy size={18} />
                  </button>
                </div>
