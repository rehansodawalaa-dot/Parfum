        ...
        {items.map(({ product, size, quantity }) => (
          <div key={`${product.id}-${size}`} className="flex gap-3">
            <img
              ...
              alt={product.name}
              className="w-14 h-16 object-cover bg-stone-50 flex-shrink-0"
            />
...
function SuccessScreen({ orderRef }) {
  return (
    <div className="min-h-screen bg-cream flex items-center justify-center pt-20 px-4">
      <div className="text-center max-w-md">
        ...
        {orderRef && (
          <p className="text-xs font-mono text-stone-400 mb-8">Order ref: {orderRef}</p>
        )}
...
export default function Checkout() {
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      // Create order on backend (if authenticated)
      let orderData = null;
      if (isAuthenticated) {
        try {
          const { data } = await api.post('/payments/create-order', {
            plan: 'starter', // adapt to your order model
            amount: total,
            items: items.map((i) => ({ id: i.product.id, qty: i.quantity, size: i.size })),
          });
          orderData = data;
        } catch {
          // Fall through to direct Razorpay if backend unavailable in demo
        }
      }
      ...
      const options = {
        ...
        handler: async (response) => {
          // Verify on backend if authenticated
          ...
          setOrderRef(response.razorpay_payment_id || 'DEMO-' + Date.now());
          clearCart();
          setSuccess(true);
        },
        modal: {
  ...
  if (success) return <SuccessScreen orderRef={orderRef} />;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
          ...
          <div className="lg:col-span-3 space-y-8">
            ...
            <button
              ...
              disabled={loading}
              className="btn-dark w-full text-base py-4"
            >
              {loading ? (
